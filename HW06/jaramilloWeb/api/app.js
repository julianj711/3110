                        ...
                        if (req.method === "POST") {
                                ...
                                req.on("end", () => {
                                        ...
                                        if (data.name && data.score !== undefined) {
                                                ...
                                                db.prepare('INSERT INTO players (name, score) VALUES (?, ?)').run(data.name, parseInt(data.score));
                                                const players = db.prepare('SELECT * FROM players').all();
                                                res.writeHead(201, { "Content-Type": "application/json" });
                                ...
                                if (player) {
                                        ...
                                        req.on("end", () => {
                                                ...
                                                if (data.score !== undefined) {
                                                        ...
                                                        db.prepare('UPDATE players SET score = ? WHERE LOWER(name) = LOWER(?)').run(parseInt(data.score), playerName);
                                                        const players = db.prepare('SELECT * FROM players').all();
                                                        res.writeHead(200, { "Content-Type": "application/json" });
