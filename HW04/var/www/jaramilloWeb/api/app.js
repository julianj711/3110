                ...
                if (req.method === "GET") {
                        ...
                        if (index !== null) {
                                ...
                                if (itemIndex >= 0 && itemIndex < items.length) {
                                        ...
                                        res.end(JSON.stringify({ player: items[itemIndex], index: itemIndex }));
                                } else {
                                ...
                                res.writeHead(200, { "Content-Type": "application/json" });
                                res.end(JSON.stringify({ players: items, count: items.length }));
                        }
                }
                ...
                // --- HANDLE POST (Create Data) ---
                        ...
                        req.on("end", () => {
                                ...
                                if (data.name && data.score !== undefined) {
                                        items.push({ name: data.name, score: parseInt(data.score) });
                                ...
                                if (itemIndex >= 0 && itemIndex < items.length) {
                                        ...
                                        req.on("end", () => {
                                                ...
                                                if (data.score !== undefined) {
                                                        //update score
                                                        items[itemIndex].score = parseInt(data.score); 
                                                        res.writeHead(200, { "Content-Type": "application/json" });
                                                        res.end(JSON.stringify({ message: "Score updated!", items: items }));
